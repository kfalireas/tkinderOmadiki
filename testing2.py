...
def create_gui(df):
	...
	def plotly_btn():
			...
			ttk.Button(new_frame, text="Ποσοστά Εμβολιασμών, Άγνωστων Κρουσμάτων και Επαναμολύνσεων", command=lambda: plotly_pie(df)).pack(pady=5,fill=tk.X)
			ttk.Button(new_frame, text="Καρτέλα αθρώπων σε κρίσιμη κατάσταση", command=lambda: plotlybar(df)).pack(pady=5,fill=tk.X)
			ttk.Button(new_frame, text="Κρουσματα, Απώλειες", command=lambda: plotly_line(df)).pack(pady=5,fill=tk.X)
			ttk.Button(new_frame, text="Linear regression", command=lambda: linear_regresion(df)).pack(pady=5,fill=tk.X)
	# CODE GIA KOUMPI
...
def plotly_line(df):
	...
	data = pd.DataFrame({'year': dates, 'krousmata': krousmata, 'apolies': apolies})

	# plotly express
	fig = px.line(data, x='year', y=['krousmata', 'apolies'], markers=True)
	fig.show()
